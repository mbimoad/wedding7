...
$('.love-story .card').each((index, item) => {
    item.dataset.aos = "fade-up"; 
})

$('.acara .card').each((index, item) => {
    item.dataset.aos = "fade-up"; 
    item.dataset.aosDelay = index * 500; 
})
...
$(window).on('scroll', function() {
    ...
    targetPosition = $('.health-protocol').offset().top; 
    if(scrollPosition > targetPosition) {
        $('.hilang').each((index, item) => {
            setTimeout(() => $('.hilang').eq(index).addClass('muncul'), index * 500); 
        })
    }
})
